...
% Reset the AI's defense to its default, in case it D'd up on its previous
% turn
...
switch player1.strategy
    ...
    case 'passive'
        ...
        elseif abs(currentPos(1)-player2.encounterPos(1)) > 2*player2.range && abs(currentPos(2)-player2.encounterPos(2)) > 2*player2.range
            player1.substrategy = 'suicidal';
        ...
        [currentPos,moveSequence] = fluicidalG(player1,player2);
        % if player1's speed is greater than 1, dUp on last move
        if player1.speed > 1
            switch moveSequence{player1.speed}
                case 'up'
    ...
    case 'neutral'
        ...
        elseif (abs(currentPos(1)-player2.encounterPos(1)) > player2.range && abs(currentPos(2)-player2.encounterPos(2))) > player2.range
            player1.substrategy = 'suicidal';
        % player 1 becomes suicidal if it is within range of attacking and
    ...
    case 'active'
        ...
        elseif (abs(currentPos(1)-player2.encounterPos(1)) > player1.range && abs(currentPos(2)-player2.encounterPos(2))) > player1.range
            player1.substrategy = 'suicidal';
        % player 1 becomes suicidal if it is within range of attacking and
...
% Counts the number of attacks made by the AI
for i=1:player1.speed
    if strcmp(moveSequence{i},'attack')
        pause(0.5);
        
        % if statement prevents attacks from occuring after the player has
        % died
        if states.inEncounter
            [states,handles,player1] = encounterAttack(states,handles,player1);
        end
    end
end

% Accounts for any D'ing up by the AI
for i=1:player1.speed
    if strcmp(moveSequence{i},'dUp')
        pause(0.5);
        
        % if statement prevents the AI from D'ing up after the player has
        % died
        if states.inEncounter
            [states,handles,player1] = encounterDUp(states,handles,player1);
        end
    end
end
